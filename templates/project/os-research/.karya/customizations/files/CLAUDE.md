...
This file and `docs/agentic-workflow.md` are collection-level, not project-level — they're synced (always overwritten from the template, same rule as above) by `templates/project/os-research/.karya/customizations/post-create.sh` regardless of *which* project under this collection you create or `--continue`. That hook runs on every `karya create project` invocation under `project/os-research/*`, walks up from the project root to find `.karya-collection`, and ensures these files exist there — so creating `xv6-c-cmake` for the first time on a fresh machine brings this file along too, not just `xv6-c-cmake`'s own content.
